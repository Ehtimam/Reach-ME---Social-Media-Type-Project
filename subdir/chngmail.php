...
class u_chng_pass
{
    ...
    function change_password($oldemail)
    {

        include('../include/connection.php');
        session_start();

        $oldemail = stripcslashes($oldemail);
        
        $oldemail = mysqli_real_escape_string($con, $oldemail);
        
        $user =  $_SESSION["logUname"];
 
        $sql = "SELECT * FROM users WHERE uname = '$user' and email = '$oldemail'";
    ...
    function mail_the_code()
    {
        ...
        $mail->IsHTML(true);
        $mail->Body    = 'ReachMe change email Verification System - please enter this code : ' . $this->class_code . '';
        $mail_rs = $mail->Send();
...
if (isset($_POST['submit'])) {
    $primOldemails = $_POST['old_mail'];
    

    $changepass = new u_chng_pass();

    $changepass->change_password($primOldemails );
